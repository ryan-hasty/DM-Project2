...
def CalculateDistance(clusters):
    centroids = np.array([c.centroid for c in clusters.cluster]) # get the centroids of all clusters
...
def MergeClosestCluster(min_row_idx, min_col_idx, clusters):
    ...
    newcentroid = np.mean([clusters.cluster[min_row_idx].centroid, clusters.cluster[min_col_idx].centroid], axis=0)
